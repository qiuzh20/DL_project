...
class Rational_search:
    ...
    def run(self, keywards, elements_num=args.num, out_type='initial'):
        ...
        end_link = time.time()
        logging.info("Finish title search in {}".format(end_link-begin_link))
        
        # get content
        data = self.spider.get_content(data)
        print("Find {} articles".format(len(data)))
        end_content = time.time()
        logging.info("Finish content search in {}".format(end_content-end_link))
        ...
        end_title = time.time()
        logging.info("Finish title processing in {}".format(end_title-end_content))
        ...
        logging.info("Finish content processing in {}".format(end_feature-end_title))

        # output results
        # TODO: support more customized output
        if out_type == 'initial':
            self.all_output(data, title_features, content_features, filename)
        elif out_type == 'emo':
            if 'emotion_cls' in self.content_process.process:
                self.emo_output(data, title_features, content_features, filename)
            else:
